...
async def test_subscription_delete(db_session):
    ...
    deleted_subscription = await subscription_repo.delete(subscription_id)

    assert deleted_subscription is not None
    assert deleted_subscription.id == subscription_id
    assert deleted_subscription.is_deleted is True

    # Verify it's marked as deleted
    fetched = await subscription_repo.get_by_id(subscription_id)
    assert fetched is None


async def test_subscription_delete_not_found(db_session):
    """Test deleting a non-existent subscription returns None."""
    subscription_repo = SubscriptionRepo(db_session)
    result = await subscription_repo.delete("non-existent-id")
...
async def test_subscription_delete_include_deleted(db_session):
    ...
    assert await subscription_repo.get_by_id(subscription_id) is None
...
async def test_subscription_multiple_users_same_topic(db_session):
    ...
    assert await subscription_repo.exists(user_id=user1.id, topic_id=topic.id) is True
    assert await subscription_repo.exists(user_id=user2.id, topic_id=topic.id) is True
    assert await subscription_repo.exists(user_id=user3.id, topic_id=topic.id) is True
...
async def test_subscription_same_user_multiple_topics(db_session):
    ...
    assert sub2.id is not None
    assert sub3.id is not None
    assert await subscription_repo.exists(user_id=user.id, topic_id=topic1.id) is True
    assert await subscription_repo.exists(user_id=user.id, topic_id=topic2.id) is True
    assert await subscription_repo.exists(user_id=user.id, topic_id=topic3.id) is True
