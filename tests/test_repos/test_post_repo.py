...
async def test_post_create(db_session):
    ...
    test_post = await post.create("test-title","test-content","test-url","test-source")
...
async def test_get_by_id(db_session):
    ...
    test_post = await post_repo.create("test-title","test-content","test-url","test-source")
...
async def test_get_by_title(db_session):
    post_repo = PostRepository(db_session)
    p1 = await post_repo.create("test-title","test-content","test-url-0","test-source")
    post_repo = PostRepository(db_session)
    p2 = await post_repo.create("test-title","test-content","test-url-1","test-source")
    p3 = await post_repo.create("test-title","test-content","test-url-2","test-source")
...
async def test_post_delete_and_include_deleted(db_session):
    post_repo = PostRepository(db_session)
    test_post = await post_repo.create("del-title","del-content","del-url","del-source")
    post_id = test_post.id
...
async def test_get_by_title_include_deleted(db_session):
    ...
    p2 = await post_repo.create("del-title","content","url-2","source")
    await post_repo.delete(p1.id)
    count, posts = await post_repo.get_by_title("del-title")
    assert count == 1
    assert len(posts) == 1
    count_all, posts_all = await post_repo.get_by_title("del-title", include_deleted=True)
    assert count_all == 2
