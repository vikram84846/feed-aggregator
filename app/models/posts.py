...
class PostTopicModel(Base,TimeStampMixin,UUIDMixin,SoftDeletedMixin):
    ...
    __table_args__ = (
        UniqueConstraint("topic_id","post_id",name="uq_post_topic"),
    )
...
class TopicModel(Base,TimeStampMixin,UUIDMixin,SoftDeletedMixin):
    ...
    users: Mapped[list["UserModel"]] = relationship("UserModel",secondary="topic_subscriptions",back_populates="topics")
    posts: Mapped[list["PostModel"]] = relationship("PostModel", secondary="post_topics",back_populates="topics")
...
class PostModel(Base,UUIDMixin,SoftDeletedMixin,TimeStampMixin):
    ...
    topics: Mapped[list["TopicModel"]] = relationship("TopicModel", secondary="post_topics",back_populates="posts")
