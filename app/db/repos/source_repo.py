...
class SourceRepository(BaseRepository):
    ...
    async def get_by_id(self, source_id:str, include_deleted:bool = False)->SourceModel | None:
        '''
        ...
        if not include_deleted:
            stmt = stmt.where(SourceModel.is_deleted == False)
        
        source = await self._session.execute(stmt)
        return source.scalar_one_or_none()
    
    async def get_by_base_url(self, base_url:str, include_deleted:bool = False)->SourceModel | None:
        '''
            fetches the source record from database
            Args:
