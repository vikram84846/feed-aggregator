...
class TopicRepository(BaseRepository):
    ...
    async def get_by_id(self, topic_id:str, include_deleted:bool=False)->TopicModel |None:
        '''
        ...
        if not include_deleted:
            stmt = stmt.where(TopicModel.is_deleted == False)
        
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()
    
    async def get_by_name(self,topic_name:str, include_deleted:bool = False)->TopicModel | None:
        '''
            fetches the topic using its id from database
            Args: 
...
class PostRepository(BaseRepository):
    ...
    async def get_by_id(self, post_id:str, include_deleted:bool = False)->PostModel | None:
        '''
        ...
        if not include_deleted:
            stmt = stmt.where(PostModel.is_deleted == False)
    ...
    async def get_by_title(
        ...
        if not include_deleted:
            base_stmt = base_stmt.where(PostModel.is_deleted == False)
